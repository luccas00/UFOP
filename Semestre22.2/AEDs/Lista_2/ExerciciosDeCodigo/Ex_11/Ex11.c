...
void insereNo (treeNode **qualquer, typeCelula aux) {
    ...
    {
        insereNo(&((*qualquer)->rightNode), aux);
    }
...
int numeroNosFolha (treeNode *raiz_x) {
    ...
    if ((raiz_x->leftNode == NULL) && (raiz_x->rightNode == NULL))
    {
        return 1;
    }

    return numeroNosFolha(raiz_x->leftNode) + numeroNosFolha(raiz_x->rightNode);
